                     ...
                     struct xmldocument *doc_old, xmlerror **error)
{
    ...
    else
    {
        ...
        for (i = 0; i < count; i++)
        {
            ...
            { /* Non empty href */
                if (!g_path_is_absolute(included_filename))
                {                                                 /* The filename's path is relative */
...
#if USE_EZXML
static void parse_node_text(ezxml_t node, void *data, void (*start)(void *, const char *, const char **, const char **, void *, void *),
