...
static char *flags_to_text(int flags)
{
    ...
    if (flags & AF_RESERVED1)
        ret = g_strconcat_printf(ret, "%sAF_RESERVED1", ret ? "|" : "");
...
int attr_from_line(const char *line, const char *name, int *pos, char *val_ret, char *name_ret)
{
    ...
    for (;;)
    {
        ...
        if (name == NULL || (e - n == len && strncmp(n, name, len) == 0))
        { /* We matched the searched attribute name */
            if (name_ret)
            { /* If instructed to, store the actual name into the string pointed by name_ret */
                len = e - n;
