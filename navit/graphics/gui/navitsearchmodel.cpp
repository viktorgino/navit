...
SearchWorker::SearchWorker(NavitInstance *navit, search_list *searchResultList, QString searchQuery, enum attr_type search_type) : m_navitInstance(navit),
...
void NavitSearchModel::setNavit(NavitInstance *navitInstance)
{
...
void NavitSearchModel::viewOnMap(SearchResult pointMap)
{
    ...
    m_navitInstance->getNavit().set_center(&pc, 1);
}
...
void NavitSearchModel::searchResultsToMap(QList<SearchResult> results)
{
    ...
    results_map_population = m_navitInstance->getNavit().populate_search_results_map(list, nullptr);
    ...
    a.u.num = 0;
    m_navitInstance->getNavit().set_attr(&a); /* Set orientation to North */
}
