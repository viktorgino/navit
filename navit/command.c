                                   ...
                                   struct attr *ret)
{
    ...
    {
        dbg(lvl_warning, "cannot retrieve attributes from %s (%p), func=%p", attr_to_name(object->type), object->u.data, func) return 0;
    }
    r = func->get_attr(object->u.data, attr_type, &dup, NULL);
    if (r)
        attr_dup_content(&dup, ret);
    else
        dbg(lvl_warning, "%s (%p) has no attribute %s", attr_to_name(object->type), object->u.data, attr_to_name(attr_type)) return r;
}
