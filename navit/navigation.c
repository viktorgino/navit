...
static struct navigation_itm *navigation_itm_new(struct navigation *this_, struct item *routeitem)
{
    ...
    if (routeitem)
    {
         ...
         * from the map.
         */
        if (ret->way.next && ((streetitem->type == type_ramp) || (streetitem->type == type_highway_land) || (streetitem->type == type_highway_city) || (streetitem->type == type_street_n_lanes)))
        {
            ...
            {
                if (rampitem->type == type_highway_exit && item_coord_get(rampitem, &exitcoord, 1) && exitcoord.x == c[0].x && exitcoord.y == c[0].y)
                {
                    while (item_attr_get(rampitem, attr_any, &attr))
                    {
                        ...
                        if (attr.type == attr_exit_to)
                        {
                             ...
                             */
                            if (attr.u.str && !ret->way.destination && (ret->way.item.type == type_ramp) && (this_->last) && (!(this_->last->way.item.type == type_ramp)))
                            {
                                char *destination_raw;
    ...
    if (!r)
    {
            ...
            if ((w->angle2 != new->way.angle2) || (w == &(new->way)))
            {
                ...
                if (is_way_allowed(nav, w, 1))
                {
                    ...
                    if ((m.num_new_motorways < 2) && is_motorway_like(w, 0))
                    {
                        ...
                        if ((w == &(new->way)) || new->way.exit_ref || new->way.exit_label || w->exit_ref || w->exit_label || !old->way.name_systematic || !new->way.name_systematic || !w->name_systematic || strcmp(old->way.name_systematic, new->way.name_systematic) || strcmp(old->way.name_systematic, w->name_systematic))
                            m.num_new_motorways++;
                    }
                ...
                if (w->flags & AF_ONEWAYMASK)
                {
                    ...
                    if (is_same_street2(new->way.name, new->way.name_systematic, w->name, w->name_systematic))
                        ...
                        through_segments++;
                    else if ((!through_segment2 && (abs(angle_delta(angle_opposite(w->angle2), new->way.angle2)) < min_turn_limit)) || (through_segment2 && (abs(angle_delta(angle_opposite(w->angle2),
        ...
        if ((abs(m.delta) >= min_turn_limit) && (through_segments >= 2))
        {
            ...
            while (ni && (hist_through_segments == 0) && (hist_dist <= junction_limit))
            {
                ...
                while (w)
                {
                    if (is_same_street2(new->way.name, new->way.name_systematic, w->name, w->name_systematic) || (through_segment2 && is_same_street2(through_segment2->name, through_segment2->name_systematic, w->name, w->name_systematic)))
        ...
        if (itm2->prev)
        {
            ...
            while (itm3->prev)
            {
                ...
                while (w2)
                {
                    ...
                    if (is_way_allowed(this_, w2, 0) && (abs(angle_delta(angle_opposite(w2->angle2), itm3->way.angle2)) <= abs(angle_delta(itm3->prev->angle_end,
                                                                                                                                           itm3->way.angle2))))
                    {
            ...
            while (itm3->next)
            {
                ...
                {
                    /* Stop examining ways at a turn maneuver (more than one way allowed and route does not follow straightest path) */
                    if (is_way_allowed(this_, w2, 0) && (abs(angle_delta(itm3->angle_end, w2->angle2)) <= abs(angle_delta(itm3->angle_end, itm3->next->way.angle2))))
                    {
                        /* FIXME: comparing angles probably does not work well for near-equal angles */
                        abort = 1;
                        break;
    ...
    if (ret->maneuver->type != type_nav_destination)
    {
        ...
        else
        {
            ...
            if (abs(ret->delta) >= min_turn_limit)
            {
                ...
                { /* next turns or bends right. Same investigation, but mirrored. */
                    while (w)
                    {
                        if (angle_delta(w->angle2, itm->angle_end) > min_turn_limit)
                        {
                                         ...
                                         char *prefix)
{
    ...
    else if (!name && !name_systematic && itm->way.item.type == type_ramp && (!cmd->maneuver || (cmd->maneuver->merge_or_exit != mex_interchange)))
    {
        ...
        if (((itm_post) && is_motorway_like(&(itm_post->way), 0)) && ((itm_pre) && !is_motorway_like(&(itm_pre->way), 0)))
...
static char *navigation_cmd_get_exit_announce(struct navigation_command *this_, char *street_destination_announce)
{
    ...
    if (this_->itm->way.exit_label)
    {
        ...
        if (!strstr(folded_street_destination_announce, folded_exit_label))
        { /* No redundancy? */
                           ...
                           enum announcement_level level)
{
    ...
    if (street_destination)
        if (level == level_connect)
        {
            ...
            if (cmd->prev)
            {
                ...
                if (street_destination_previous && strstr(street_destination_previous, street_destination))
    ...
    if (cmd->maneuver && cmd->maneuver->type)
    {
        ...
        if (!(cmd->maneuver->merge_or_exit == mex_none))
        {
            ...
            case mex_exit_left:
            case mex_exit_right:
                direction = (cmd->maneuver->merge_or_exit == mex_exit_left) ? g_strdup(_("on your left")) : g_strdup(_("on your right"));
                if (exit_announce)
                                 ...
                                 enum attr_type type)
{
    ...
    if (cmd->next && cmd->itm)
    {
        ...
        if (nextlevel <= level_soon)
        {
            ...
            if (*next != '\0')
            { /* is the second announcement not an empty string? */
                cmd->itm->told = 1;
                buf = ret;
