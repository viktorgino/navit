                                 ...
                                 struct coord *c, int count)
{
    ...
    while ((curr = map_rect_get_item(mr)) && !ret)
    {
        ...
        if (curr_priv->coords[0].x == c[0].x && curr_priv->coords[0].y == c[0].y && curr_priv->coords[curr_priv->coord_count - 1].x == c[count - 1].x && curr_priv->coords[curr_priv->coord_count - 1].y == c[count - 1].y)
        {
        ...
        else if (curr_priv->coords[0].x == c[count - 1].x && curr_priv->coords[0].y == c[count - 1].y && curr_priv->coords[curr_priv->coord_count - 1].x == c[0].x && curr_priv->coords[curr_priv->coord_count - 1].y == c[0].y)
        {
            if (wanted_flags_attr.type == attr_none)
            {
                /* no flag comparison, match */
...
static int traffic_event_is_valid(struct traffic_event *this_)
{
    ...
    case event_class_restriction:
        if ((this_->type < event_restriction_access_restrictions_lifted) || (this_->type > event_restriction_speed_limit_lifted))
        {
    ...
    if ((this_->road_type != type_line_unspecified))
    {
        ...
        if (item->type == this_->road_type)
            ...
            case type_street_n_lanes:
                if ((item->type == type_highway_land) || (item->type == type_highway_city) || (item->type == type_street_4_land) || (item->type == type_street_4_city))
                    score += 200;
                break;
            /* primary */
...
static int traffic_location_get_point_triple(struct traffic_location *this_, struct coord_geo **coords)
{
    ...
    else if (this_->not_via)
    {
        ...
        if (transform_distance(projection_mg, &c_from, &c_not_via) < transform_distance(projection_mg, &c_to, &c_not_via))
        {
        ...
        { /* Iterating all the segments leading away from our point to update the points at their ends */
            ...
            if (val != INT_MAX)
            {
                ...
                { /* We've found a less costly way to reach the end of s, update it */
                    ...
                    if (new < start_value)
                    { /* We've found a less costly way from the start point, update */
                        start_value = new;
                        ret = s->end;
                    }
                }
                                        ...
                                        struct map *map, struct route *route)
{
    ...
    while (1)
    { /* once for each direction (loop logic at the end) */
        ...
        else if (point_pairs == 2)
        {
            ...
            if ((this_->location->fuzziness == location_fuzziness_low_res) || this_->location->at || this_->location->not_via)
            {
        ...
        if ((this_->location->fuzziness == location_fuzziness_low_res) || this_->location->at || this_->location->not_via)
        {
            ...
            if ((p_from == p_to) || !p_from->seg)
            {
                ...
                if (p_from->seg)
                {
                    ...
                    if (transform_distance(projection_mg, &p_to->c, pcoords[1] ? pcoords[1] : pcoords[2]) > transform_distance(projection_mg, &p_iter->c, pcoords[1] ? pcoords[1] : pcoords[2]))
                    {
                        p_to = p_from;
                        p_from = p_iter;
                    }
        ...
        while (s)
        {
            ...
            else
            {
                /* backward direction */
                p_iter = s->start;
                flags = data->flags | (s->data.flags & AF_ONEWAYMASK) | (data->dir == location_dir_one ? AF_ONEWAYREV : 0);
            }
         ...
         this_->shared->message_queue = g_list_remove(this_->shared->message_queue, message))
    {
        ...
        else
        {
            ...
            if (!message->is_cancellation)
            {
                ...
                {
                    stored_msg = (struct traffic_message *)msg_iter->data;
                    if (seg_data_equals(data, traffic_message_parse_events(stored_msg)) && traffic_location_equals(message->location, stored_msg->location))
                ...
                else
                {
                    ...
                    if (route_get_attr(this_->shared->rt, attr_route_status, &attr, NULL) && route_get_pos(this_->shared->rt) && ((attr.u.num & route_status_destination_set)))
                    {
                        traffic_location_set_enclosing_rect(message->location, NULL);
...
static int traffic_xml_is_tagstack_valid(struct xml_state *state)
{
    ...
    for (tagiter = g_list_last(state->tagstack); tagiter; tagiter = g_list_previous(tagiter))
    {
        ...
        if (!g_ascii_strcasecmp(el->tag_name, "navit_messages") || !g_ascii_strcasecmp(el->tag_name, "feed"))
            ...
            ret = (!el_parent || !g_ascii_strcasecmp(el_parent->tag_name, "navit_messages") || !g_ascii_strcasecmp(el_parent->tag_name, "feed"));
        else if (!g_ascii_strcasecmp(el->tag_name, "events") || !g_ascii_strcasecmp(el->tag_name, "location") || !g_ascii_strcasecmp(el->tag_name, "merge"))
        ...
        else if (!g_ascii_strcasecmp(el->tag_name, "from") || !g_ascii_strcasecmp(el->tag_name, "to") || !g_ascii_strcasecmp(el->tag_name, "at") || !g_ascii_strcasecmp(el->tag_name, "via") || !g_ascii_strcasecmp(el->tag_name, "not_via") || !g_ascii_strcasecmp(el->tag_name, "navit_items"))
